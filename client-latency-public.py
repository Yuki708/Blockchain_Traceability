...
def create_transaction(user_id, operation="upload_asset"):
    data_id = str(random.getrandbits(256))
    data = {
        "AssetID": data_id,
        "UserID": user_id,
        "RandomData": random.getrandbits(256)
    }
...
def send_block():
    ...
    txn = create_transaction(user_id, operation="upload_asset")
    ...
    # 提交批处理
    try:
        response = requests.post(NODE_URLS[0] + '/batches', data=batch_list_bytes)
